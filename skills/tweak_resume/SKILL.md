...
- `/see:tweak_resume` - Start the flow (will ask for a job URL)
- `/see:tweak_resume https://...` - Tailor resume for a specific job posting
...
3. **Gap identification**: Note any requirements where the candidate has no clear match. Categorize each gap:
   - **Hard gap**: A specific credential or certification the candidate doesn't have (e.g., "CPA required")
   - **Soft gap**: A skill or experience area where the candidate might have unlisted experience (e.g., "experience with data pipelines", "public speaking")
...
If the match analysis found **soft gaps**, present them to the user before writing the resume:

```
This role asks for a few things I don't see on your resume. Before I write
the tailored version, do you have experience with any of these?

1. **[Gap skill/area]** — The job asks for [what the posting says].
   Do you have any experience with this — even from side projects,
   a previous career, volunteering, or self-teaching?

2. **[Gap skill/area]** — They want [what the posting says].
   Have you done anything related, even if it wasn't your main job?
```

For each gap the candidate can fill:
- Capture the experience (what they did, the context, any results)
- Add it to `DATA_DIR/resume/parsed.md` skills inventory and `DATA_DIR/profile.md` (under Non-Resume Experience if it wasn't from a listed role)
- Use it in the tailored resume

For gaps they can't fill, note them in the tailoring notes so the candidate knows what to address in a cover letter or interview prep.

For **hard gaps** (credentials they don't have), don't ask — just note them in the output.
...
- Business model corrections (e.g., "Acme Corp is B2C, not B2B")
